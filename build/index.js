...
class GitHubProjectsServer {
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Projects for ${owner}:\n\n${projects.map((project) => `• ${project.title} (#${project.number})\n` +
                            `  ID: ${project.id}\n` +
                ...
                content: [
                    {
                        ...
                        text: `Project Details:\n\n` +
                            ...
                            `Status: ${project.closed ? 'Closed' : 'Open'}\n` +
                            `URL: ${project.url}\n\n` +
                            `Fields (${project.fields?.nodes?.length || 0}):\n${(project.fields?.nodes || []).map((field) => `• ${field.name} (${field.dataType}) - ID: ${field.id}\n`).join('')}\n` +
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Item added to project successfully!\n\nItem ID: ${result.addProjectV2ItemById.item.id}`,
                    },
                ],
            };
        }
        ...
        try {
            ...
            if (!actualFieldType) {
                try {
                    ...
                    actualFieldType = field?.dataType;
                }
                catch {
            ...
            switch (actualFieldType) {
                ...
                case "SINGLE_SELECT":
                    ...
                    try {
                        ...
                        const project = projectResult.node;
                        const field = project?.fields?.nodes?.find((f) => f.id === fieldId);
                        const option = field?.options?.find((opt) => opt.name.toLowerCase() === value.toLowerCase());
                        if (option) {
                            formattedValue = { singleSelectOptionId: option.id };
                        }
                        else {
                            throw new Error(`Option '${value}' not found for field. Available options: ${field?.options?.map((opt) => opt.name).join(', ')}`);
                        }
                    }
                    catch (error) {
                ...
                content: [
                    {
                        ...
                        text: `Project item field updated successfully!\n\n` +
                            `Item ID: ${result.updateProjectV2ItemFieldValue.projectV2Item.id}\n` +
    ...
    async updateItemStatus(args) {
        ...
        try {
            ...
            const statusField = project.fields?.nodes?.find((field) => field.name.toLowerCase().includes('status') ||
            ...
            if (!statusField) {
                throw new Error("No status field found. Available fields: " +
                    (project.fields?.nodes?.map((f) => f.name).join(', ') || 'none'));
            }
            // Find the option by name
            const option = statusField.options?.find((opt) => opt.name.toLowerCase() === status.toLowerCase());
            if (!option) {
                throw new Error(`Status '${status}' not found. Available statuses: ${statusField.options?.map((opt) => opt.name).join(', ')}`);
            }
            // Update the status
            const result = await this.graphqlClient(GRAPHQL_QUERIES.UPDATE_PROJECT_ITEM_FIELD, {
                projectId,
            ...
            const fieldDetails = fields.map((field) => {
                ...
                if (field.dataType === 'SINGLE_SELECT' && field.options) {
                    ...
                    field.options.forEach((option, index) => {
                        ...
                        if (option.color)
                            fieldInfo += ` 🎨${option.color}`;
                        if (option.description)
                            fieldInfo += ` - ${option.description}`;
                        fieldInfo += '\n';
                    });
                }
                ...
                if (field.dataType === 'ITERATION' && field.configuration?.iterations) {
                    ...
                    field.configuration.iterations.forEach((iteration, index) => {
                        fieldInfo += `    ${index + 1}. "${iteration.title}" (${iteration.duration} days)`;
                        if (iteration.startDate)
                            fieldInfo += ` - Starts: ${new Date(iteration.startDate).toLocaleDateString()}`;
                        fieldInfo += '\n';
                    });
                }
                return fieldInfo;
                ...
                if (content) {
                    if (content.__typename === 'Issue' || content.__typename === 'PullRequest') {
                        ...
                        if (content.assignees?.nodes?.length > 0) {
                            itemInfo += `   - Assignees: ${content.assignees.nodes.map((a) => `@${a.login}`).join(', ')}\n`;
                        }
                    }
                    else if (content.__typename === 'DraftIssue') {
                    ...
                    item.fieldValues.nodes.forEach((fieldValue) => {
                        if (fieldValue.field?.name) {
                            ...
                            if (fieldValue.text !== undefined)
                                ...
                                value = new Date(fieldValue.date).toLocaleDateString();
